...
public class Client {
   ...
   public static void main(String[] args) throws FileNotFoundException {
      ...
      Scanner tudorFamily = new Scanner(new File("tudor.dat")); // Scanner that reads the tudor.dat file with valid
                                                                // names
   ...
   public static void run(Scanner console, Scanner tudorFamily) throws FileNotFoundException {
      ...
      if (name.equalsIgnoreCase("STOP")) {
         ...
         // Search through tudor.dat file for inputted name and create instance of Person
         // object if found
         while (tudorFamily.hasNextLine()) {
            String line = tudorFamily.nextLine();
            Scanner lineScan = new Scanner(line);

            while (lineScan.hasNext() && !(line.equals("END"))) { // For all person names (stopping at first "END" -
                                                                  // excludes family names & duplicates)
   ...
   // Written by Melina Perraut
   // Checks whether the user wants to search for another name, and runs the
